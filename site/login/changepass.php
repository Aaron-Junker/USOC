...
session_start();
  ...
  if(isset($_POST["oldpass"])&&isset($_POST["newpass1"])&&isset($_POST["newpass2"])&&isset($_SESSION['User_Name'])){
    if($_POST["newpass1"]==$_POST["newpass2"]&&preg_match('/^[a-z0-9A-Z.:,;]{8,25}$/',$_POST["newpass1"])){
      ...
      while ($zeile = mysqli_fetch_array( $db_erg, MYSQLI_ASSOC))
        {
          if(md5($zeile["Id"]) == $_SESSION['User_ID']&&$zeile["Password"]==password_hash($_POST["oldpass"],PASSWORD_DEFAULT,["salt"=>"\@]FBGsU,P?eFbHb)!'uaIt95JYhlH%TrRf+rZt|/~0T){%DjBasjhklozu"])){
            $passc = True;
          }
        }
    }else{
  ...
  if($passc){
    ...
    $sql = "UPDATE User SET password='".password_hash($_POST["oldpass"],PASSWORD_DEFAULT,["salt"=>"\@]FBGsU,P?eFbHb)!'uaIt95JYhlH%TrRf+rZt|/~0T){%DjBasjhklozu"])."' WHERE Id='".$_SESSION['User_ID']."';";
    echo $sql;
    $db_erg = mysqli_query( $db_link, $sql );
